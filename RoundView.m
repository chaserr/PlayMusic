...
#define RotationDuration 8.0

@interface RoundView ()

@property (nonatomic, retain) UIImageView *roundImageView;//外环
@property (nonatomic, retain) UIImageView *playStateView;// 内环
...
- (void)initRoundView{
    ...
    UIImage *startImage;
    if (self.isPlay) {
        startImage = [UIImage imageNamed:@"start"];
    }else{
        
        startImage =[UIImage imageNamed:@"pause"];
    }
    self.playStateView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, startImage.size.width, startImage.size.height)];
    [self.playStateView setCenter:center];
    [self.playStateView setImage:startImage];
    [self addSubview:self.playStateView];
    
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    CGContextRef context = CGBitmapContextCreate(nil, self.frame.size.width, self.frame.size.height, 8, 0, colorSpace, kCGBitmapByteOrderDefault);
    ...
    if (self.rotationDuration == 0) {
        self.rotationDuration = _rotationDuration;
    }
...
-(void) startRotation
{
    ...
    [UIView animateWithDuration:1.0 delay:0 options:UIViewAnimationOptionCurveEaseInOut
                     ...
                     completion:^(BOOL finished){
                         if (finished){
                             [UIView animateWithDuration:1.0 animations:^{self.playStateView.alpha = 1;}];
                         }
                     }
     ];
